...
def fetch_all_records():
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT bbm.id_masuk, sbb.nama_bahan_baku, sbb.satuan_bahan_baku, bbm.tanggal, bbm.jumlah_bahan_baku, bbm.id_bahan_baku
        FROM bahan_baku_masuk bbm
        JOIN stok_bahan_baku sbb ON bbm.id_bahan_baku = sbb.id_bahan_baku
        ORDER BY bbm.tanggal DESC
    """)
    rows = cur.fetchall()
    con.close()
    return rows
...
def add_record(data):
    ...
    cur = con.cursor()
    cur.execute(
        'INSERT INTO bahan_baku_masuk (id_bahan_baku, tanggal, jumlah_bahan_baku) VALUES (?, ?, ?)',
        data
    )
    con.commit()
    con.close()
...
def update_record(id_masuk, data):
    con = sqlite3.connect("inventory.db")
    cur = con.cursor()
    cur.execute(
        'UPDATE bahan_baku_masuk SET id_bahan_baku = ?, tanggal = ?, jumlah_bahan_baku = ? WHERE id_masuk = ?',
        (*data, id_masuk)
    )
    con.commit()
    con.close()
...
def delete_record(id_masuk):
    con = sqlite3.connect("inventory.db")
    cur = con.cursor()
    cur.execute('DELETE FROM bahan_baku_masuk WHERE id_masuk = ?', (id_masuk,))
    con.commit()
    con.close()
...
# -----------------------------
with tab1:
    st.subheader("Tambah Data Bahan Baku Masuk")

    if st.button('➕ Tambah Data Masuk'):
        st.session_state.show_add_dialog = True

        @st.dialog("Form Tambah Bahan Baku Masuk")
        def tambah_dialog():
...
# -----------------------------
with tab2:
    st.subheader("Daftar Bahan Baku Masuk")
    data = fetch_all_records()

    if not data:
        st.info("Belum ada data bahan baku masuk.")
    else:
        # Header kolom
        header_cols = st.columns([1, 2, 1, 1, 1])
        header_cols[0].markdown("**Tanggal Masuk**")
        ...
        for record in data:
            id_masuk, nama, satuan, tanggal, jumlah, id_bahan_baku = record
            ...
            with btn_edit:
                if st.button("✏️", key=f"edit_{id_masuk}", help="Edit"):
                    st.session_state.dialog_type = "edit"
                    st.session_state.active_record = record
            with btn_delete:
                if st.button("🗑️", key=f"delete_{id_masuk}", help="Hapus"):
                    st.session_state.dialog_type = "delete"
                    st.session_state.active_record = record

        @st.dialog("Form Data Bahan Baku Masuk")
        def universal_dialog(record, mode):
            ...
            if mode == "edit":
                ...
                new_bahan = st.selectbox("Pilih Bahan Baku", list(
                    bahan_dict.keys()), index=list(bahan_dict.keys()).index(current_option))
                ...
                new_tanggal = st.date_input("Tanggal Masuk", value=date_value)
                new_jumlah = st.number_input(
                    "Jumlah Bahan Baku", min_value=0.0, step=0.1, value=float(jumlah))

                if st.button("💾 Simpan Perubahan"):
                    update_record(
                        id_masuk, (bahan_dict[new_bahan], str(new_tanggal), new_jumlah))
