...
def fetch_all_records():
    ...
    cur = con.cursor()
    cur.execute("""
        SELECT pjm.id_masuk, sbj.nama_barang_jadi, sbj.grade_barang_jadi, sbj.satuan_barang_jadi,
               pjm.tanggal, pjm.jumlah_barang_jadi
        FROM produk_jadi_masuk pjm
        JOIN stok_barang_jadi sbj ON pjm.id_barang_jadi = sbj.id_barang_jadi
        ORDER BY pjm.tanggal DESC
    """)
    rows = cur.fetchall()
    con.close()
    return rows
...
def add_record(data):
    ...
    cur = con.cursor()
    cur.execute(
        'INSERT INTO produk_jadi_masuk (id_barang_jadi, tanggal, jumlah_barang_jadi) VALUES (?, ?, ?)',
        data
    )
    con.commit()
    con.close()
...
def update_record(id_masuk, data):
    ...
    cur = con.cursor()
    cur.execute(
        'UPDATE produk_jadi_masuk SET id_barang_jadi = ?, tanggal = ?, jumlah_barang_jadi = ? WHERE id_masuk = ?',
        (*data, id_masuk)
    )
    con.commit()
    con.close()
...
def delete_record(id_masuk):
    con = sqlite3.connect("inventory.db")
    cur = con.cursor()
    cur.execute('DELETE FROM produk_jadi_masuk WHERE id_masuk = ?', (id_masuk,))
    con.commit()
    con.close()
...
# -----------------------------
with tab1:
    st.subheader("Tambah Data Produk Jadi Masuk")

    if st.button('➕ Tambah Data Masuk'):
        st.session_state.show_add_dialog = True

        @st.dialog("Form Tambah Produk Jadi Masuk")
        def tambah_dialog():
...
# -----------------------------
with tab2:
    st.subheader("Daftar Produk Jadi Masuk")
    data = fetch_all_records()

    if not data:
        st.info("Belum ada data produk jadi masuk.")
    else:
        # Header kolom
        ...
        # Baris data
        for record in data:
            id_masuk, nama, grade, satuan, tanggal, jumlah = record
        ...
        # -----------------------------
        @st.dialog("Form Data Produk Jadi Masuk")
        def universal_dialog(record, mode):
            ...
            if mode == "edit":
                ...
                new_tanggal = st.date_input(
                    "Tanggal Masuk", pd.to_datetime(tanggal))
                new_jumlah = st.number_input(
                    "Jumlah Produk Jadi", min_value=0.0, step=0.1, value=float(jumlah))

                if st.button("💾 Simpan Perubahan"):
                    update_record(
                        id_masuk, (produk_dict[new_produk], str(new_tanggal), new_jumlah))
